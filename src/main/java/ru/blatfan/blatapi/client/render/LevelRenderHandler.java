...
public class LevelRenderHandler {
    ...
    public static void standardDelayedRender(RenderLevelStageEvent event) {
        if (event.getStage() == RenderLevelStageEvent.Stage.AFTER_WEATHER) {
            ...
            for (RenderType renderType : BARenderTypes.translucentRenderTypes) getDelayedRender().endBatch(renderType);
            RenderSystem.getModelViewMatrix().set(last);
            for (RenderType renderType : BARenderTypes.translucentParticleRenderTypes) getDelayedRender().endBatch(renderType);
            if (MATRIX4F != null) RenderSystem.getModelViewMatrix().set(MATRIX4F);
            for (RenderType renderType : BARenderTypes.additiveRenderTypes) getDelayedRender().endBatch(renderType);
            RenderSystem.getModelViewMatrix().set(last);
            for (RenderType renderType : BARenderTypes.additiveParticleRenderTypes) getDelayedRender().endBatch(renderType);
        }
    }
    ...
    public static MultiBufferSource.BufferSource getDelayedRender() {
        if (DELAYED_RENDER == null) {
            ...
            for (RenderType type : BARenderTypes.renderTypes) {
