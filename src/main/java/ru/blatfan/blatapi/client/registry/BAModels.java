...
import ru.blatfan.blatapi.client.model.item.BowItemOverrides;
import ru.blatfan.blatapi.client.model.item.CustomItemOverrides;
import ru.blatfan.blatapi.client.model.item.CustomModel;
import ru.blatfan.blatapi.client.model.item.CustomRenderModel;

import java.util.ArrayList;
import java.util.Map;

public class BAModels {
    ...
    public static ModelLayerLocation addLayer(String layer) {
        return addLayer(BlatApi.MOD_ID, layer);
    }
    ...
    public static void addBowItemModel(Map<ResourceLocation, BakedModel> map, ResourceLocation item, BowItemOverrides itemOverrides) {
        ...
        for (int i = 0; i < 3; i++) {
            BakedModel pullModel = map.get(new ModelResourceLocation(ResourceLocation.parse(item + "_pulling_" + i), "inventory"));
    ...
    public static ArrayList<ModelResourceLocation> getBowModels(String modId, String item) {
        ...
        models.add(new ModelResourceLocation(ResourceLocation.fromNamespaceAndPath(modId, item), "inventory"));
        models.add(new ModelResourceLocation(ResourceLocation.fromNamespaceAndPath(modId, item + "_pulling_0"), "inventory"));
        models.add(new ModelResourceLocation(ResourceLocation.fromNamespaceAndPath(modId, item + "_pulling_1"), "inventory"));
        models.add(new ModelResourceLocation(ResourceLocation.fromNamespaceAndPath(modId, item + "_pulling_2"), "inventory"));
        return models;
    }
