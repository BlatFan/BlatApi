package ru.blatfan.blatapi.common.network.item;
...
import ru.blatfan.blatapi.client.particle.behavior.*;
import ru.blatfan.blatapi.client.particle.data.*;
...
import ru.blatfan.blatapi.client.screenshake.ScreenshakeHandler;
import ru.blatfan.blatapi.client.screenshake.ScreenshakeInstance;
...
public class TestStickPacket extends TwoPositionClientPacket {
    protected final int mode;

    public TestStickPacket(double x1, double y1, double z1, double x2, double y2, double z2, int mode) {
        super(x1, y1, z1, x2, y2, z2);
        this.mode = mode;
    }

    public TestStickPacket(Vec3 vec1, Vec3 vec2, int mode) {
        super(vec1, vec2);
        this.mode = mode;
    }
    ...
    public void execute(Supplier<NetworkEvent.Context> context) {
        ...
        if (mode == 2) {
            Vec3 pos = startPos.add(lookPos.scale(5f));
            ParticleBuilder.create(BARegistry.Particles.WISP)
                    .setRenderType(BARenderTypes.TRANSLUCENT_PARTICLE)
                    .setColorData(ColorParticleData.create(0, 0, 1, 1, 0, 0).build())
                    .setTransparencyData(GenericParticleData.create(0.5f, 0).setEasing(Easing.QUARTIC_OUT).build())
                    .setScaleData(GenericParticleData.create(0.3f, 2, 0).setEasing(Easing.ELASTIC_OUT).build())
        ...
        if (mode == 6) {
            Vec3 pos = startPos.add(lookPos.scale(5f));
            ParticleBuilder.create(BARegistry.Particles.DOT)
        ...
        if (mode == 7) {
            ...
            ParticleBuilder.create(BARegistry.Particles.WISP)
                    ...
                    .repeat(level, pos.x(), pos.y(), pos.z(), 50);
            level.playSound(BlatApi.proxy.getPlayer(), pos.x(), pos.y(), pos.z(), SoundEvents.RESPAWN_ANCHOR_CHARGE, SoundSource.PLAYERS, 1f, 1f);
        }
        
        if (mode == 8) {
            Vec3 pos = startPos.add(lookPos.scale(0.75f));
            ItemParticleOptions options = new ItemParticleOptions(BARegistry.Particles.ITEM.get(), new ItemStack(BARegistry.Items.TEST_STICK.get()));
        ...
        if (mode == 10) {
            Vec3 pos = startPos.add(lookPos.scale(0.75f));
            FluidParticleOptions options = new FluidParticleOptions(BARegistry.Particles.FLUID.get(), new FluidStack(Fluids.LAVA, 1), false);
            ParticleBuilder.create(options)
                    .setRenderType(BARenderTypes.TRANSLUCENT_BLOCK_PARTICLE)
                    .setColorData(ColorParticleData.create(Color.WHITE).build())
                    .setTransparencyData(GenericParticleData.create(0.5f, 0).setEasing(Easing.QUARTIC_OUT).build())
        ...
        if (mode == 13) {
            ...
            ParticleBuilder.create(BARegistry.Particles.PANCAKE)
                    .setRenderType(BARenderTypes.TRANSLUCENT_PARTICLE)
                    .setColorData(ColorParticleData.create(Color.WHITE).build())
        ...
        if (mode == 14) {
            Vec3 pos = startPos.add(lookPos.scale(3f));
            ParticleBuilder.create(BARegistry.Particles.DEATH)
                    .setRenderType(BARenderTypes.TRANSLUCENT_PARTICLE)
                    .setColorData(ColorParticleData.create(Color.WHITE).build())
                    .setTransparencyData(GenericParticleData.create(1f, 0).setEasing(Easing.QUARTIC_OUT).build())
                    .setScaleData(GenericParticleData.create(0.3f, 2, 0).setEasing(Easing.ELASTIC_OUT).build())
        ...
        if (mode == 17) {
            Vec3 pos = startPos.add(lookPos.scale(5f));
            ParticleBuilder.create(BARegistry.Particles.CHERRY_LEAVES)
        ...
        if (mode == 20) {
            Vec3 pos = startPos.add(lookPos.scale(5f));
            ParticleBuilder.create(BARegistry.Particles.TRAIL)
                    .setRenderType(BARenderTypes.TRANSLUCENT_PARTICLE_TEXTURE)
        ...
        if (mode == 21) {
            Vec3 pos = startPos.add(lookPos.scale(5f));
            ParticleBuilder builder = ParticleBuilder.create(BARegistry.Particles.WISP)
                    .setColorData(ColorParticleData.create(0, 0, 1, 1, 0, 0).build())
        ...
        if (mode == 23) {
            Vec3 pos = startPos.add(lookPos.scale(5f));
            ParticleBuilder.create(BARegistry.Particles.SKULL)
                    .setColorData(ColorParticleData.create(0, 0, 1, 1, 0, 0).build())
                    .setTransparencyData(GenericParticleData.create(1, 1, 0).setEasing(Easing.QUARTIC_OUT).build())
                    .setScaleData(GenericParticleData.create(0.05f, 0.1f, 0).setEasing(Easing.ELASTIC_OUT).build())
                    .setLifetime(20, 10)
        ...
        if (mode == 24) {
            ...
            ParticleBuilder.create(BARegistry.Particles.EARTH)
                    .setRenderType(BARenderTypes.TRANSLUCENT_PARTICLE_TEXTURE)
        ...
        if (mode == 25) {
            Vec3 pos = startPos.add(lookPos.scale(10f));
            ParticleBuilder.create(BARegistry.Particles.SUN)
                    .setRenderType(BARenderTypes.TRANSLUCENT_PARTICLE_TEXTURE)
                    .setBehavior(SphereParticleBehavior.create().disableSecondSide().setSphereSize(64, 32).build())
                    .setColorData(ColorParticleData.create(Color.WHITE).build())
                    .setTransparencyData(GenericParticleData.create(1).build())
    ...
    public static void register(SimpleChannel instance, int index) {
        instance.registerMessage(index, TestStickPacket.class, TestStickPacket::encode, TestStickPacket::decode, TestStickPacket::handle);
    }
    ...
    public static TestStickPacket decode(FriendlyByteBuf buf) {
        return new TestStickPacket(buf.readDouble(), buf.readDouble(), buf.readDouble(), buf.readDouble(), buf.readDouble(), buf.readDouble(), buf.readInt());
    }
}
