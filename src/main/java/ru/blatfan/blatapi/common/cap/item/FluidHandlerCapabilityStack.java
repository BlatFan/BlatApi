...
public class FluidHandlerCapabilityStack implements IFluidHandlerItem, ICapabilityProvider {
  ...
  public FluidStack drain(int maxDrain, FluidAction action) {
    ...
    FluidStack contained = getFluid();
    if (contained.isEmpty() || !canDrainFluidType(contained)) return FluidStack.EMPTY;
    final int drainAmount = Math.min(contained.getAmount(), maxDrain);
    FluidStack drained = contained.copy();
    drained.setAmount(drainAmount);
    if (action.execute()) {
      contained.shrink(drainAmount);
      if (contained.isEmpty()) setContainerToEmpty();
  ...
  public <T> LazyOptional<T> getCapability(Capability<T> capability, Direction facing) {
    if (ForgeCapabilities.FLUID_HANDLER_ITEM == capability) return holder.cast();
