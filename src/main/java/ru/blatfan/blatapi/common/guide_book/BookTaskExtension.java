...
public class BookTaskExtension implements BookGuiExtension {
    @Override
    public boolean shouldRender(GuideBookEntry entry) {
        return !entry.completed(GuideClient.player);
    }
    
    @Override
    public void render(GuideBookData data, GuideBookEntry entry, GuiGraphics gui, int x, int y, int mX, int mY, float pTick) {
        renderBG(gui, data, x, y);
